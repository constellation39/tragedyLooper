// --- effect.proto ---

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: v1/effect.proto

package v1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ForbidEffect_ForbidType int32

const (
	ForbidEffect_FORBID_TYPE_UNSPECIFIED ForbidEffect_ForbidType = 0 // 未指定
	ForbidEffect_MOVEMENT                ForbidEffect_ForbidType = 1 // 禁止移动
	ForbidEffect_PARANOIA_CHANGE         ForbidEffect_ForbidType = 2 // 禁止妄想值变化
	ForbidEffect_GOODWILL_CHANGE         ForbidEffect_ForbidType = 3 // 禁止好感值变化
	ForbidEffect_INTRIGUE_CHANGE         ForbidEffect_ForbidType = 4 // 禁止阴谋值变化
)

// Enum value maps for ForbidEffect_ForbidType.
var (
	ForbidEffect_ForbidType_name = map[int32]string{
		0: "FORBID_TYPE_UNSPECIFIED",
		1: "MOVEMENT",
		2: "PARANOIA_CHANGE",
		3: "GOODWILL_CHANGE",
		4: "INTRIGUE_CHANGE",
	}
	ForbidEffect_ForbidType_value = map[string]int32{
		"FORBID_TYPE_UNSPECIFIED": 0,
		"MOVEMENT":                1,
		"PARANOIA_CHANGE":         2,
		"GOODWILL_CHANGE":         3,
		"INTRIGUE_CHANGE":         4,
	}
)

func (x ForbidEffect_ForbidType) Enum() *ForbidEffect_ForbidType {
	p := new(ForbidEffect_ForbidType)
	*p = x
	return p
}

func (x ForbidEffect_ForbidType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ForbidEffect_ForbidType) Descriptor() protoreflect.EnumDescriptor {
	return file_v1_effect_proto_enumTypes[0].Descriptor()
}

func (ForbidEffect_ForbidType) Type() protoreflect.EnumType {
	return &file_v1_effect_proto_enumTypes[0]
}

func (x ForbidEffect_ForbidType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ForbidEffect_ForbidType.Descriptor instead.
func (ForbidEffect_ForbidType) EnumDescriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{3, 0}
}

// 效果，定义了能力、卡牌或规则的具体作用
type Effect struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to EffectType:
	//
	//	*Effect_AdjustStat
	//	*Effect_MoveCharacter
	//	*Effect_Forbid
	//	*Effect_GrantAbility
	//	*Effect_RevealRole
	//	*Effect_ChangeRole
	//	*Effect_TriggerIncident
	//	*Effect_EndGame
	//	*Effect_AddTrait
	//	*Effect_RemoveTrait
	EffectType    isEffect_EffectType `protobuf_oneof:"effect_type"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Effect) Reset() {
	*x = Effect{}
	mi := &file_v1_effect_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Effect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Effect) ProtoMessage() {}

func (x *Effect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Effect.ProtoReflect.Descriptor instead.
func (*Effect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{0}
}

func (x *Effect) GetEffectType() isEffect_EffectType {
	if x != nil {
		return x.EffectType
	}
	return nil
}

func (x *Effect) GetAdjustStat() *AdjustStatEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_AdjustStat); ok {
			return x.AdjustStat
		}
	}
	return nil
}

func (x *Effect) GetMoveCharacter() *MoveCharacterEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_MoveCharacter); ok {
			return x.MoveCharacter
		}
	}
	return nil
}

func (x *Effect) GetForbid() *ForbidEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_Forbid); ok {
			return x.Forbid
		}
	}
	return nil
}

func (x *Effect) GetGrantAbility() *GrantAbilityEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_GrantAbility); ok {
			return x.GrantAbility
		}
	}
	return nil
}

func (x *Effect) GetRevealRole() *RevealRoleEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_RevealRole); ok {
			return x.RevealRole
		}
	}
	return nil
}

func (x *Effect) GetChangeRole() *ChangeRoleEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_ChangeRole); ok {
			return x.ChangeRole
		}
	}
	return nil
}

func (x *Effect) GetTriggerIncident() *TriggerIncidentEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_TriggerIncident); ok {
			return x.TriggerIncident
		}
	}
	return nil
}

func (x *Effect) GetEndGame() *EndGameEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_EndGame); ok {
			return x.EndGame
		}
	}
	return nil
}

func (x *Effect) GetAddTrait() *AddTraitEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_AddTrait); ok {
			return x.AddTrait
		}
	}
	return nil
}

func (x *Effect) GetRemoveTrait() *RemoveTraitEffect {
	if x != nil {
		if x, ok := x.EffectType.(*Effect_RemoveTrait); ok {
			return x.RemoveTrait
		}
	}
	return nil
}

type isEffect_EffectType interface {
	isEffect_EffectType()
}

type Effect_AdjustStat struct {
	AdjustStat *AdjustStatEffect `protobuf:"bytes,1,opt,name=adjust_stat,json=adjustStat,proto3,oneof"` // 调整属性
}

type Effect_MoveCharacter struct {
	MoveCharacter *MoveCharacterEffect `protobuf:"bytes,2,opt,name=move_character,json=moveCharacter,proto3,oneof"` // 移动角色
}

type Effect_Forbid struct {
	Forbid *ForbidEffect `protobuf:"bytes,3,opt,name=forbid,proto3,oneof"` // 禁止行动
}

type Effect_GrantAbility struct {
	GrantAbility *GrantAbilityEffect `protobuf:"bytes,4,opt,name=grant_ability,json=grantAbility,proto3,oneof"` // 授予能力
}

type Effect_RevealRole struct {
	RevealRole *RevealRoleEffect `protobuf:"bytes,5,opt,name=reveal_role,json=revealRole,proto3,oneof"` // 揭示身份
}

type Effect_ChangeRole struct {
	ChangeRole *ChangeRoleEffect `protobuf:"bytes,6,opt,name=change_role,json=changeRole,proto3,oneof"` // 改变身份
}

type Effect_TriggerIncident struct {
	TriggerIncident *TriggerIncidentEffect `protobuf:"bytes,7,opt,name=trigger_incident,json=triggerIncident,proto3,oneof"` // 触发事件
}

type Effect_EndGame struct {
	EndGame *EndGameEffect `protobuf:"bytes,8,opt,name=end_game,json=endGame,proto3,oneof"` // 结束游戏
}

type Effect_AddTrait struct {
	AddTrait *AddTraitEffect `protobuf:"bytes,9,opt,name=add_trait,json=addTrait,proto3,oneof"` // 添加特征
}

type Effect_RemoveTrait struct {
	RemoveTrait *RemoveTraitEffect `protobuf:"bytes,10,opt,name=remove_trait,json=removeTrait,proto3,oneof"` // 移除特征
}

func (*Effect_AdjustStat) isEffect_EffectType() {}

func (*Effect_MoveCharacter) isEffect_EffectType() {}

func (*Effect_Forbid) isEffect_EffectType() {}

func (*Effect_GrantAbility) isEffect_EffectType() {}

func (*Effect_RevealRole) isEffect_EffectType() {}

func (*Effect_ChangeRole) isEffect_EffectType() {}

func (*Effect_TriggerIncident) isEffect_EffectType() {}

func (*Effect_EndGame) isEffect_EffectType() {}

func (*Effect_AddTrait) isEffect_EffectType() {}

func (*Effect_RemoveTrait) isEffect_EffectType() {}

// 调整属性效果
type AdjustStatEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"`                                                     // 目标角色
	StatType      StatCondition_StatType `protobuf:"varint,2,opt,name=stat_type,json=statType,proto3,enum=v1.StatCondition_StatType" json:"stat_type,omitempty"` // 调整的属性类型
	Amount        int32                  `protobuf:"varint,3,opt,name=amount,proto3" json:"amount,omitempty"`                                                    // 调整量（正数增加，负数减少）
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AdjustStatEffect) Reset() {
	*x = AdjustStatEffect{}
	mi := &file_v1_effect_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AdjustStatEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AdjustStatEffect) ProtoMessage() {}

func (x *AdjustStatEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AdjustStatEffect.ProtoReflect.Descriptor instead.
func (*AdjustStatEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{1}
}

func (x *AdjustStatEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *AdjustStatEffect) GetStatType() StatCondition_StatType {
	if x != nil {
		return x.StatType
	}
	return StatCondition_STAT_TYPE_UNSPECIFIED
}

func (x *AdjustStatEffect) GetAmount() int32 {
	if x != nil {
		return x.Amount
	}
	return 0
}

// 移动角色效果
type MoveCharacterEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"`                                 // 目标角色
	Destination   LocationType           `protobuf:"varint,2,opt,name=destination,proto3,enum=v1.LocationType" json:"destination,omitempty"` // 目标地点
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MoveCharacterEffect) Reset() {
	*x = MoveCharacterEffect{}
	mi := &file_v1_effect_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MoveCharacterEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MoveCharacterEffect) ProtoMessage() {}

func (x *MoveCharacterEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MoveCharacterEffect.ProtoReflect.Descriptor instead.
func (*MoveCharacterEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{2}
}

func (x *MoveCharacterEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *MoveCharacterEffect) GetDestination() LocationType {
	if x != nil {
		return x.Destination
	}
	return LocationType_LOCATION_TYPE_UNSPECIFIED
}

// 禁止效果
type ForbidEffect struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Target        *TargetSelector         `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"`                                                            // 目标角色
	ForbidType    ForbidEffect_ForbidType `protobuf:"varint,2,opt,name=forbid_type,json=forbidType,proto3,enum=v1.ForbidEffect_ForbidType" json:"forbid_type,omitempty"` // 禁止的行动类型
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ForbidEffect) Reset() {
	*x = ForbidEffect{}
	mi := &file_v1_effect_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ForbidEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ForbidEffect) ProtoMessage() {}

func (x *ForbidEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ForbidEffect.ProtoReflect.Descriptor instead.
func (*ForbidEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{3}
}

func (x *ForbidEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *ForbidEffect) GetForbidType() ForbidEffect_ForbidType {
	if x != nil {
		return x.ForbidType
	}
	return ForbidEffect_FORBID_TYPE_UNSPECIFIED
}

// 授予能力效果
type GrantAbilityEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"`                               // 目标角色
	AbilityId     int32                  `protobuf:"varint,2,opt,name=ability_id,json=abilityId,proto3" json:"ability_id,omitempty"`       // 授予的能力ID
	IsTemporary   bool                   `protobuf:"varint,3,opt,name=is_temporary,json=isTemporary,proto3" json:"is_temporary,omitempty"` // 是否是临时能力（例如，只持续一天或一循环）
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GrantAbilityEffect) Reset() {
	*x = GrantAbilityEffect{}
	mi := &file_v1_effect_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GrantAbilityEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GrantAbilityEffect) ProtoMessage() {}

func (x *GrantAbilityEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GrantAbilityEffect.ProtoReflect.Descriptor instead.
func (*GrantAbilityEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{4}
}

func (x *GrantAbilityEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *GrantAbilityEffect) GetAbilityId() int32 {
	if x != nil {
		return x.AbilityId
	}
	return 0
}

func (x *GrantAbilityEffect) GetIsTemporary() bool {
	if x != nil {
		return x.IsTemporary
	}
	return false
}

// 揭示身份效果
type RevealRoleEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"` // 目标角色
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RevealRoleEffect) Reset() {
	*x = RevealRoleEffect{}
	mi := &file_v1_effect_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevealRoleEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevealRoleEffect) ProtoMessage() {}

func (x *RevealRoleEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevealRoleEffect.ProtoReflect.Descriptor instead.
func (*RevealRoleEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{5}
}

func (x *RevealRoleEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

// 改变身份效果
type ChangeRoleEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"`                                    // 目标角色
	NewRole       RoleType               `protobuf:"varint,2,opt,name=new_role,json=newRole,proto3,enum=v1.RoleType" json:"new_role,omitempty"` // 新的身份
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeRoleEffect) Reset() {
	*x = ChangeRoleEffect{}
	mi := &file_v1_effect_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeRoleEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeRoleEffect) ProtoMessage() {}

func (x *ChangeRoleEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeRoleEffect.ProtoReflect.Descriptor instead.
func (*ChangeRoleEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{6}
}

func (x *ChangeRoleEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *ChangeRoleEffect) GetNewRole() RoleType {
	if x != nil {
		return x.NewRole
	}
	return RoleType_ROLE_TYPE_UNSPECIFIED
}

// 触发事件效果
type TriggerIncidentEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IncidentId    int32                  `protobuf:"varint,1,opt,name=incident_id,json=incidentId,proto3" json:"incident_id,omitempty"` // 要触发的事件ID (引用ScriptConfig中的IncidentConfig)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TriggerIncidentEffect) Reset() {
	*x = TriggerIncidentEffect{}
	mi := &file_v1_effect_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TriggerIncidentEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TriggerIncidentEffect) ProtoMessage() {}

func (x *TriggerIncidentEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TriggerIncidentEffect.ProtoReflect.Descriptor instead.
func (*TriggerIncidentEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{7}
}

func (x *TriggerIncidentEffect) GetIncidentId() int32 {
	if x != nil {
		return x.IncidentId
	}
	return 0
}

// 结束游戏效果
type EndGameEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Winner        PlayerRole             `protobuf:"varint,1,opt,name=winner,proto3,enum=v1.PlayerRole" json:"winner,omitempty"` // 宣布的胜利方
	Reason        string                 `protobuf:"bytes,2,opt,name=reason,proto3" json:"reason,omitempty"`                     // 游戏结束原因描述
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EndGameEffect) Reset() {
	*x = EndGameEffect{}
	mi := &file_v1_effect_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EndGameEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EndGameEffect) ProtoMessage() {}

func (x *EndGameEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EndGameEffect.ProtoReflect.Descriptor instead.
func (*EndGameEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{8}
}

func (x *EndGameEffect) GetWinner() PlayerRole {
	if x != nil {
		return x.Winner
	}
	return PlayerRole_PLAYER_ROLE_UNSPECIFIED
}

func (x *EndGameEffect) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

// 添加特征效果
type AddTraitEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"` // 目标角色
	Trait         string                 `protobuf:"bytes,2,opt,name=trait,proto3" json:"trait,omitempty"`   // 要添加的特征
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddTraitEffect) Reset() {
	*x = AddTraitEffect{}
	mi := &file_v1_effect_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddTraitEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddTraitEffect) ProtoMessage() {}

func (x *AddTraitEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddTraitEffect.ProtoReflect.Descriptor instead.
func (*AddTraitEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{9}
}

func (x *AddTraitEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *AddTraitEffect) GetTrait() string {
	if x != nil {
		return x.Trait
	}
	return ""
}

// 移除特征效果
type RemoveTraitEffect struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Target        *TargetSelector        `protobuf:"bytes,1,opt,name=target,proto3" json:"target,omitempty"` // 目标角色
	Trait         string                 `protobuf:"bytes,2,opt,name=trait,proto3" json:"trait,omitempty"`   // 要移除的特征
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveTraitEffect) Reset() {
	*x = RemoveTraitEffect{}
	mi := &file_v1_effect_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveTraitEffect) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveTraitEffect) ProtoMessage() {}

func (x *RemoveTraitEffect) ProtoReflect() protoreflect.Message {
	mi := &file_v1_effect_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveTraitEffect.ProtoReflect.Descriptor instead.
func (*RemoveTraitEffect) Descriptor() ([]byte, []int) {
	return file_v1_effect_proto_rawDescGZIP(), []int{10}
}

func (x *RemoveTraitEffect) GetTarget() *TargetSelector {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *RemoveTraitEffect) GetTrait() string {
	if x != nil {
		return x.Trait
	}
	return ""
}

var File_v1_effect_proto protoreflect.FileDescriptor

const file_v1_effect_proto_rawDesc = "" +
	"\n" +
	"\x0fv1/effect.proto\x12\x02v1\x1a\x0ev1/enums.proto\x1a\x12v1/condition.proto\"\xd6\x04\n" +
	"\x06Effect\x127\n" +
	"\vadjust_stat\x18\x01 \x01(\v2\x14.v1.AdjustStatEffectH\x00R\n" +
	"adjustStat\x12@\n" +
	"\x0emove_character\x18\x02 \x01(\v2\x17.v1.MoveCharacterEffectH\x00R\rmoveCharacter\x12*\n" +
	"\x06forbid\x18\x03 \x01(\v2\x10.v1.ForbidEffectH\x00R\x06forbid\x12=\n" +
	"\rgrant_ability\x18\x04 \x01(\v2\x16.v1.GrantAbilityEffectH\x00R\fgrantAbility\x127\n" +
	"\vreveal_role\x18\x05 \x01(\v2\x14.v1.RevealRoleEffectH\x00R\n" +
	"revealRole\x127\n" +
	"\vchange_role\x18\x06 \x01(\v2\x14.v1.ChangeRoleEffectH\x00R\n" +
	"changeRole\x12F\n" +
	"\x10trigger_incident\x18\a \x01(\v2\x19.v1.TriggerIncidentEffectH\x00R\x0ftriggerIncident\x12.\n" +
	"\bend_game\x18\b \x01(\v2\x11.v1.EndGameEffectH\x00R\aendGame\x121\n" +
	"\tadd_trait\x18\t \x01(\v2\x12.v1.AddTraitEffectH\x00R\baddTrait\x12:\n" +
	"\fremove_trait\x18\n" +
	" \x01(\v2\x15.v1.RemoveTraitEffectH\x00R\vremoveTraitB\r\n" +
	"\veffect_type\"\x8f\x01\n" +
	"\x10AdjustStatEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x127\n" +
	"\tstat_type\x18\x02 \x01(\x0e2\x1a.v1.StatCondition.StatTypeR\bstatType\x12\x16\n" +
	"\x06amount\x18\x03 \x01(\x05R\x06amount\"u\n" +
	"\x13MoveCharacterEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x122\n" +
	"\vdestination\x18\x02 \x01(\x0e2\x10.v1.LocationTypeR\vdestination\"\xf0\x01\n" +
	"\fForbidEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x12<\n" +
	"\vforbid_type\x18\x02 \x01(\x0e2\x1b.v1.ForbidEffect.ForbidTypeR\n" +
	"forbidType\"v\n" +
	"\n" +
	"ForbidType\x12\x1b\n" +
	"\x17FORBID_TYPE_UNSPECIFIED\x10\x00\x12\f\n" +
	"\bMOVEMENT\x10\x01\x12\x13\n" +
	"\x0fPARANOIA_CHANGE\x10\x02\x12\x13\n" +
	"\x0fGOODWILL_CHANGE\x10\x03\x12\x13\n" +
	"\x0fINTRIGUE_CHANGE\x10\x04\"\x82\x01\n" +
	"\x12GrantAbilityEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x12\x1d\n" +
	"\n" +
	"ability_id\x18\x02 \x01(\x05R\tabilityId\x12!\n" +
	"\fis_temporary\x18\x03 \x01(\bR\visTemporary\">\n" +
	"\x10RevealRoleEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\"g\n" +
	"\x10ChangeRoleEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x12'\n" +
	"\bnew_role\x18\x02 \x01(\x0e2\f.v1.RoleTypeR\anewRole\"8\n" +
	"\x15TriggerIncidentEffect\x12\x1f\n" +
	"\vincident_id\x18\x01 \x01(\x05R\n" +
	"incidentId\"O\n" +
	"\rEndGameEffect\x12&\n" +
	"\x06winner\x18\x01 \x01(\x0e2\x0e.v1.PlayerRoleR\x06winner\x12\x16\n" +
	"\x06reason\x18\x02 \x01(\tR\x06reason\"R\n" +
	"\x0eAddTraitEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x12\x14\n" +
	"\x05trait\x18\x02 \x01(\tR\x05trait\"U\n" +
	"\x11RemoveTraitEffect\x12*\n" +
	"\x06target\x18\x01 \x01(\v2\x12.v1.TargetSelectorR\x06target\x12\x14\n" +
	"\x05trait\x18\x02 \x01(\tR\x05traitB)Z'tragedylooper/internal/game/proto/v1;v1b\x06proto3"

var (
	file_v1_effect_proto_rawDescOnce sync.Once
	file_v1_effect_proto_rawDescData []byte
)

func file_v1_effect_proto_rawDescGZIP() []byte {
	file_v1_effect_proto_rawDescOnce.Do(func() {
		file_v1_effect_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_v1_effect_proto_rawDesc), len(file_v1_effect_proto_rawDesc)))
	})
	return file_v1_effect_proto_rawDescData
}

var file_v1_effect_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_v1_effect_proto_msgTypes = make([]protoimpl.MessageInfo, 11)
var file_v1_effect_proto_goTypes = []any{
	(ForbidEffect_ForbidType)(0),  // 0: v1.ForbidEffect.ForbidType
	(*Effect)(nil),                // 1: v1.Effect
	(*AdjustStatEffect)(nil),      // 2: v1.AdjustStatEffect
	(*MoveCharacterEffect)(nil),   // 3: v1.MoveCharacterEffect
	(*ForbidEffect)(nil),          // 4: v1.ForbidEffect
	(*GrantAbilityEffect)(nil),    // 5: v1.GrantAbilityEffect
	(*RevealRoleEffect)(nil),      // 6: v1.RevealRoleEffect
	(*ChangeRoleEffect)(nil),      // 7: v1.ChangeRoleEffect
	(*TriggerIncidentEffect)(nil), // 8: v1.TriggerIncidentEffect
	(*EndGameEffect)(nil),         // 9: v1.EndGameEffect
	(*AddTraitEffect)(nil),        // 10: v1.AddTraitEffect
	(*RemoveTraitEffect)(nil),     // 11: v1.RemoveTraitEffect
	(*TargetSelector)(nil),        // 12: v1.TargetSelector
	(StatCondition_StatType)(0),   // 13: v1.StatCondition.StatType
	(LocationType)(0),             // 14: v1.LocationType
	(RoleType)(0),                 // 15: v1.RoleType
	(PlayerRole)(0),               // 16: v1.PlayerRole
}
var file_v1_effect_proto_depIdxs = []int32{
	2,  // 0: v1.Effect.adjust_stat:type_name -> v1.AdjustStatEffect
	3,  // 1: v1.Effect.move_character:type_name -> v1.MoveCharacterEffect
	4,  // 2: v1.Effect.forbid:type_name -> v1.ForbidEffect
	5,  // 3: v1.Effect.grant_ability:type_name -> v1.GrantAbilityEffect
	6,  // 4: v1.Effect.reveal_role:type_name -> v1.RevealRoleEffect
	7,  // 5: v1.Effect.change_role:type_name -> v1.ChangeRoleEffect
	8,  // 6: v1.Effect.trigger_incident:type_name -> v1.TriggerIncidentEffect
	9,  // 7: v1.Effect.end_game:type_name -> v1.EndGameEffect
	10, // 8: v1.Effect.add_trait:type_name -> v1.AddTraitEffect
	11, // 9: v1.Effect.remove_trait:type_name -> v1.RemoveTraitEffect
	12, // 10: v1.AdjustStatEffect.target:type_name -> v1.TargetSelector
	13, // 11: v1.AdjustStatEffect.stat_type:type_name -> v1.StatCondition.StatType
	12, // 12: v1.MoveCharacterEffect.target:type_name -> v1.TargetSelector
	14, // 13: v1.MoveCharacterEffect.destination:type_name -> v1.LocationType
	12, // 14: v1.ForbidEffect.target:type_name -> v1.TargetSelector
	0,  // 15: v1.ForbidEffect.forbid_type:type_name -> v1.ForbidEffect.ForbidType
	12, // 16: v1.GrantAbilityEffect.target:type_name -> v1.TargetSelector
	12, // 17: v1.RevealRoleEffect.target:type_name -> v1.TargetSelector
	12, // 18: v1.ChangeRoleEffect.target:type_name -> v1.TargetSelector
	15, // 19: v1.ChangeRoleEffect.new_role:type_name -> v1.RoleType
	16, // 20: v1.EndGameEffect.winner:type_name -> v1.PlayerRole
	12, // 21: v1.AddTraitEffect.target:type_name -> v1.TargetSelector
	12, // 22: v1.RemoveTraitEffect.target:type_name -> v1.TargetSelector
	23, // [23:23] is the sub-list for method output_type
	23, // [23:23] is the sub-list for method input_type
	23, // [23:23] is the sub-list for extension type_name
	23, // [23:23] is the sub-list for extension extendee
	0,  // [0:23] is the sub-list for field type_name
}

func init() { file_v1_effect_proto_init() }
func file_v1_effect_proto_init() {
	if File_v1_effect_proto != nil {
		return
	}
	file_v1_enums_proto_init()
	file_v1_condition_proto_init()
	file_v1_effect_proto_msgTypes[0].OneofWrappers = []any{
		(*Effect_AdjustStat)(nil),
		(*Effect_MoveCharacter)(nil),
		(*Effect_Forbid)(nil),
		(*Effect_GrantAbility)(nil),
		(*Effect_RevealRole)(nil),
		(*Effect_ChangeRole)(nil),
		(*Effect_TriggerIncident)(nil),
		(*Effect_EndGame)(nil),
		(*Effect_AddTrait)(nil),
		(*Effect_RemoveTrait)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_v1_effect_proto_rawDesc), len(file_v1_effect_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   11,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_v1_effect_proto_goTypes,
		DependencyIndexes: file_v1_effect_proto_depIdxs,
		EnumInfos:         file_v1_effect_proto_enumTypes,
		MessageInfos:      file_v1_effect_proto_msgTypes,
	}.Build()
	File_v1_effect_proto = out.File
	file_v1_effect_proto_goTypes = nil
	file_v1_effect_proto_depIdxs = nil
}
